## Tide(潮汐)
Eventually, all data meets the shore.  
潮汐有涨有落，但最终会覆盖整个海岸线。数据可能有短暂的不一致，但最终会达到一致性。
...
```SQL
CREATE DATABASE `tide` CHARACTER SET 'utf8' COLLATE 'utf8mb4_general_ci';

USE `tide`;

CREATE TABLE `tide_tend_consistency_task` (
...
## 4、安装tide-core到本地maven仓库

在idea中，右键tide-core > open in terminal > 输入：mvn clean install
...
```xml
<dependency>
    <groupId>com.github.tide</groupId>
    <artifactId>tide-core</artifactId>
...
spring:
  ...
  shardingsphere:
    ...
    sharding:
      tables:
        # 最终一致性任务库分库策略
        tide_tend_consistency_task:
          # 数据分布情况
          actual-data-nodes: ds$->{0..1}.tide_tend_consistency_task
          # order库分库策略
          database-strategy:
            inline:
...
- 1、创建一个实现了com.github.tide.custom.query.TaskTimeRangeQuery接口的类,比如我们的实现类为 MyTaskTimeRangeQuery
- 2、在spring-boot工程（业务服务工程）中的application.yml或者application.properties中加入配置
  github.tide.parallel.pool.task-schedule-time-range-class-name=com.github.tide.demo.range.MyTaskTimeRangeQuery
- 可定制的查询条件有：
...
>
> - 实现的具体示例： 参考示例工程tide-springboot-demo下的com.github.tide.demo.range.MyTaskTimeRangeQuery类
>
...
- 1、创建一个实现了com.github.tide.custom.shard.ShardingKeyGenerator接口的类，比如我们这里的实现类为：MySnowflakeShardingKeyGenerator
- 2、在spring-boot工程（业务服务工程）中的application.yml或者application.properties中加入配置
  github.tide.shard.sharding-key-generator-class-name=com.github.tide.demo.shard.SnowflakeShardingKeyGenerator
- 实现的方法介绍:
...
-
2、实现com.github.tide.custom.alerter.ConsistencyFrameworkAlerter接口，并在类上标注@Component，这里标注@Component是因为框架使用了spring容器来获取具体实现类，并执行相关逻辑
