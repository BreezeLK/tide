package com.github.tide.mapper;
...
public interface TaskStoreMapper {
     ...
     * @return 存储结果
     */
    @Insert("INSERT INTO tide_tend_consistency_task("
                + "task_id,"
                + "task_status,"
                + "execute_times,"
                + "execute_time,"
    ...
    @Select("SELECT " +
            ...
            "FROM tide_tend_consistency_task " +
            "where " +
     ...
     * @return 标记结果
     */
    @Delete("DELETE FROM tide_tend_consistency_task WHERE id=#{id} and shard_key=#{shardKey}")
     ...
     * @return 标记结果
     */
    @Update("UPDATE tide_tend_consistency_task SET task_status=2, error_msg=#{errorMsg}, execute_time=#{executeTime} WHERE id=#{id} and shard_key=#{shardKey}")
     ...
     * @return 标记结果
     */
    @Update("UPDATE tide_tend_consistency_task SET fallback_error_msg=#{fallbackErrorMsg} WHERE id=#{id} and shard_key=#{shardKey}")
